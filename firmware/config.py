...
CHIP_ID_REQUEST = 0x66
# CH552 datasheet 6.7: most-significant byte first; skip reserved 0x3FFB.
CHIP_UID_ADDRESSES = (0x3FFA, 0x3FFF, 0x3FFE, 0x3FFD, 0x3FFC)
SETTINGS_SCHEMA = 1
...
# Accept existing boards so the host tools can perform their firmware upgrade.
USB_MANUFACTURERS = (USB_MANUFACTURER, "CreatorKanata")
...
#define CHIP_UID_ADDRESSES {{{", ".join(hex(a) for a in CHIP_UID_ADDRESSES)}}}
...
#define SAVE_LED_MS {SAVE_LED_MS}
...
# Browser settings save polling; no local server is used.
WEB_SAVE_TIMEOUT_MS = 5000
WEB_SAVE_POLL_MS = 20
